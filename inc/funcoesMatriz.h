...
// Escreve um número na matriz de LEDs.
void writeNumberToMatrix(char number, npLED_t leds[]) {

  if (number < '0' || number > '9') {
    return; // Número fora do intervalo permitido (0-9)
  }

  int index = number - '0';

  for (uint i = 0; i < LED_COUNT; ++i) {
    uint32_t color = numbers_matrix[index][i];
    uint8_t r = (color >> 16) & 0xFF;
    uint8_t g = (color >> 8) & 0xFF;
    uint8_t b = color & 0xFF;
    setMatrizLED(i, r, g, b, leds);
  }
  matrizWrite(leds);
}